...
# 路由列表,每一个元素就是一个路由，手动添加路由到路由列表(后续Django的方式)
# route装饰器装饰的时候就把所有的url -> view handler 映射好了，即route_list被初始化
route_list = [
    # ('/index.html', index),
    # ('/center.html', center)
    # ('/center_data.html', center_data)
]


# 定义一个带参数的装饰器,将路由添加到路由列表(后续flask的方式)
def route(path):
    def decorator(func):
        # 当装饰的时候就要把路由添加到路由列表
        ...
        def inner():
            # route_list.append(path, func)      # 注意：不能放在这，否则被装饰函数调用一次就要添加一次，也就是请求刷新一次网页就会添加一次路由
            return func()
...
def index():
    ...
    # web框架处理后的数据
    #   1.打开模板文件并读取
    with open('template/index.html', 'r', encoding='utf-8') as f:  # 这里读的仅是html文件，不会有图片等超文本，所以不需以rb方式读取，且windows里要指定utf-8编码！
        file_data = f.read()
...
def center():
    ...
    # web框架处理后的数据
    #   1.打开模板文件并读取
    with open('template/center.html', 'r',
              encoding='utf-8') as file:  # 这里读的仅是html文件，不会有图片等超文本，所以不需以rb方式读取，且需要指定utf-8编码！
        file_data = file.read()
